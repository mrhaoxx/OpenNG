...
func StdForwardProxy(ctx *HttpCtx) Ret {
	...
	if ctx.Req.Method == "CONNECT" {
		...
		if err != nil {
			ctx.ErrorPage(http.StatusBadRequest, fmt.Sprintf("Dial: %v", err))
			return RequestEnd
		}
		defer server.Close()
		...
		if ctx.Req.ProtoMajor == 0 || ctx.Req.ProtoMajor == 1 {
			...
			if err != nil {
				ctx.ErrorPage(http.StatusBadRequest, fmt.Sprintf("Hijack: %v", err))
				return RequestEnd
			}
		...
		if err != nil {
			ctx.ErrorPage(http.StatusBadRequest, fmt.Sprintf("%v", err))
			return RequestEnd
		}
