...
func (h *StdForwardProxy) HandleHTTPForward(ctx *nghttp.HttpCtx) nghttp.Ret {
	...
	if ctx.Req.Method == "CONNECT" {
		...
		if err != nil {
			ctx.Resp.ErrorPage(nghttp.StatusBadRequest, fmt.Sprintf("Forward: Dial: %v", err))
		...
		case 0, 1:
			...
			if err != nil {
				ctx.Resp.ErrorPage(nghttp.StatusBadRequest, fmt.Sprintf("Forward: Hijack: %v", err))
				return nghttp.RequestEnd
			}
...
var _ nghttp.Forward = (*StdForwardProxy)(nil)
